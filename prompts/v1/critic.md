...
- 必须检查 bullet 是否把 JD 当作注释拼进了简历正文。
  - 如果出现 `JD`、`岗位要求`、`任职要求`、`加分项`、`对应某项要求`、`符合岗位要求` 之类表达，应视为明显质量问题。
  - 反馈要明确指出：JD 只能指导改写方向，不应直接出现在简历 bullet 中。
- 必须检查 bullet 是否虽然没有硬拼 JD，但仍然像 AI 匹配稿而不像简历。
  - 如果 bullet 只是在说“具备某能力”“支持业务推进”“积累相关经验”，却没有讲清动作、对象、方法或结果，应指出。
  - 如果 bullet 过于抽象、只有能力词没有具体场景，也应指出。
